...
from experimental_experiment.torch_models.mixtral_helper import get_mixtral_model
...
class TestEdMixtral(ExtTestCase):

    @unittest.skipIf(sys.platform == "win32", reason="not supported yet on Windows")
    @ignore_warnings(DeprecationWarning)
    @requires_torch("2.3", "AssertionError: original output #6 is None")
    def test_mixtral_export_rename(self):
        model, input_tensors = get_mixtral_model()
    ...
    @ignore_warnings(DeprecationWarning)
    @requires_torch("2.3", "AssertionError: original output #6 is None")
    def test_mixtral_export_norename(self):
        model, input_tensors = get_mixtral_model()
        input_tensors = input_tensors[0]
        expected = model(*input_tensors)
    ...
    @requires_torch("2.3", "AssertionError: original output #6 is None")
    def test_mixtral_cort_static(self):
        import torch
        ...
        if __name__ == "__main__":
            for i, inst in enumerate(instances):
                self.dump_onnx(f"test_mixtral_cort_static_{i}.onnx", inst["onnx"])

    @unittest.skipIf(sys.platform == "win32", reason="not supported yet on Windows")
    @ignore_warnings((DeprecationWarning, UserWarning))
    @requires_torch("2.3", "AssertionError: original output #6 is None")
    def test_mixtral_cort_static_norename(self):
        model, input_tensors = get_mixtral_model()
        input_tensors = input_tensors[0]
        expected = model(*input_tensors)
        ...
        if __name__ == "__main__":
            for i, inst in enumerate(instances):
                self.dump_onnx(
                    f"test_mixtral_cort_static_{i}_norename.onnx", inst["onnx"]
                )
    ...
    @unittest.skipIf(
        True,
        reason="torch._dynamo.exc.Unsupported: dynamic "
        "shape operator: aten.nonzero.default;",
    )
    def test_mixtral_cort_dynamic(self):
        model, input_tensors = get_mixtral_model()
        input_tensors = input_tensors[0]
        expected = model(*input_tensors)
        ...
        if __name__ == "__main__":
            for i, inst in enumerate(instances):
                self.dump_onnx(f"test_mixtral_cort_dynamic_{i}.onnx", inst["onnx"])
    ...
    def test_mixtral_cort_dynamic_norename(self):
        model, input_tensors = get_mixtral_model()
        input_tensors = input_tensors[0]
        expected = model(*input_tensors)
        ...
        if __name__ == "__main__":
            for i, inst in enumerate(instances):
                self.dump_onnx(
                    f"test_mixtral_cort_dynamic_{i}_norename.onnx", inst["onnx"]
                )
