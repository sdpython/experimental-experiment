...
class TestOnnxExportTracing(ExtTestCase):
    ...
    def test_tracing_dynamic_cache2(self):
        ...
        class Model(torch.nn.Module):
            ...
            def forward(self, x, y, cache: DynamicCache):
                return self.sub(x, self.subcache(cache)) + y

        model = Model()
        inputs = (
            torch.rand((5, 6, 5, 6)),
            torch.rand((5, 6, 5, 6)),
            make_dynamic_cache([(torch.ones((5, 6, 5, 6)), torch.ones((5, 6, 5, 6)) + 2)]),
        )
        expected = model(*inputs)
        with torch_export_patches(patch_transformers=True):
            onx = to_onnx(
