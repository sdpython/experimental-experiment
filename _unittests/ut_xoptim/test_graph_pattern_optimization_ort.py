...
class TestGraphPatternOptimizationOrt(ExtTestCase):
    ...
    def test_multi_head_attention_switch_where(self):
        # Tests the LocalAttentionSW_to variant: switch_where=True (Where(mask, -inf, 0))
        model = oh.make_model(
            oh.make_graph(
                [
                    oh.make_node("Transpose", ["query"], ["t_query"], perm=[0, 2, 1, 3]),
                    ...
                    oh.make_node("MatMul", ["query_scaled", "keys_scaled_t"], ["qk"]),
                    oh.make_node("Where", ["mask", "minfty", "zero"], ["bias"]),
                    oh.make_node("Add", ["qk", "bias"], ["qkb"]),
                    oh.make_node("Softmax", ["qkb"], ["qkbs"], axis=-1),
                    oh.make_node("IsNaN", ["qkbs"], ["nans"]),
                    oh.make_node("Where", ["nans", "zero", "qkbs"], ["filt"]),
        ...
        opt_onx = gr.to_onnx(optimize=True)
        self.dump_onnx("test_multi_head_attention_switch_where.onnx", opt_onx)
        self.assertIn("MultiHeadAttention", [n.op_type for n in opt_onx.graph.node])
        ref2 = self.make_inference_session(opt_onx)
        zz = ref2.run(None, feeds)[0]
        self.assertEqualArray(z, zz, atol=1e-4)

    def test_multi_head_attention_fused_matmul_switch_where(self):
        # Tests the LocalAttentionSWNoT_to variant: switch_where=True + FusedMatMul (transB=1)
        model = oh.make_model(
            oh.make_graph(
                [
                    oh.make_node("Transpose", ["query"], ["t_query"], perm=[0, 2, 1, 3]),
        ...
        opt_onx = gr.to_onnx(optimize=True)
        self.dump_onnx("test_multi_head_attention_fused_matmul_switch_where.onnx", opt_onx)
        self.assertIn("MultiHeadAttention", [n.op_type for n in opt_onx.graph.node])
        ref2 = self.make_inference_session(opt_onx)
        zz = ref2.run(None, feeds)[0]
        self.assertEqualArray(z, zz, atol=1e-4)
