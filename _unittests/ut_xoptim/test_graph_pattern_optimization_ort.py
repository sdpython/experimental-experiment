...
class TestGraphPatternOptimizationOrt(ExtTestCase):
    ...
    def test_multi_head_attention_fused_matmul(self):
        # Test that MultiHeadAttention3DPattern handles the noT variant produced
        # when FusedMatMulPattern (com.microsoft) runs before FunctionAttentionPattern.
        # The model uses FusedMatMul instead of Transpose+MatMul for the QK computation.
        model = oh.make_model(
            oh.make_graph(
                [
                    oh.make_node("Transpose", ["query"], ["t_query"], perm=[0, 2, 1, 3]),
