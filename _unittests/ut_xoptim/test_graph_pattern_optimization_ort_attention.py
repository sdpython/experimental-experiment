...
class TestGraphPatternOptimizationOrtAttention(ExtTestCase):
    ...
    def _get_model_attention_1(self):
        ...
        initializers.append(onh.from_array(np.array(0.125, dtype=np.float32), name="val_122"))
        initializers.append(
        ...
        inputs.append(
            oh.make_tensor_value_info(
                "expand_1",
                TensorProto.BOOL,
                shape=("s0", "CeilToInt(IntTrueDiv(s1, 8))", "CeilToInt(IntTrueDiv(s1, 8))"),
            )
        )
        inputs.append(
            oh.make_tensor_value_info(
        ...
        nodes.append(make_node_extended("Mul", ["transpose_1", "val_122"], ["mul_209"]))
        nodes.append(
            make_node_extended("Transpose", ["view_1"], ["transpose_2"], perm=[0, 2, 1, 3])
        )
        nodes.append(
        ...
        nodes.append(
            make_node_extended(
                "Transpose", ["transpose_2"], ["transpose_4"], perm=[0, 1, 3, 2]
            )
        )
        nodes.append(make_node_extended("MatMul", ["mul_209", "transpose_4"], ["matmul"]))
        ...
        outputs.append(
            oh.make_tensor_value_info(
                "view_3", TensorProto.FLOAT, shape=("s0", "CeilToInt(IntTrueDiv(s1, 8))", 1024)
            )
        )
        graph = oh.make_graph(
            nodes,
    ...
    def test_attention_pattern_1(self):
        ...
        gr = GraphBuilder(
            ...
            optimization_options=OptimizationOptions(patterns=["Attention"], verbose=10),
        )
