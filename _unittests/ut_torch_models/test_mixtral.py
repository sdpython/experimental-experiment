...
class TestMixtral(ExtTestCase):
    ...
    @unittest.skipIf(
        True,
        reason="orch._dynamo.exc.Unsupported: dynamic "
        "shape operator: aten.nonzero.default;",
    )
    def test_ort_mixtral_model(self):
        from experimental_experiment.torch_models.mixtral_helper import (
            get_mixtral_model,
        )

        input_dims = self.get_input_dims(False)
        model, example_args_collection = get_mixtral_model(input_dims=input_dims)
        self.common_test_model(
            ...
            dynamic=False,
            fullgraph=True,
            onnx_export="test_ort_mixtral_model",
            expected_graph_break=0,
        )
    ...
    def test_ort_mixtral_model_cuda(self):
        from experimental_experiment.torch_models.mixtral_helper import (
            get_mixtral_model,
        )
    ...
    @skipif_ci_windows("torch.compile not supported on Windows")
    @requires_torch("2.3", "missing kernel")
    @unittest.skipIf(True, reason="issue with dynamic shapes")
    def test_ort_mixtral_model_backward(self):
        from experimental_experiment.torch_models.mixtral_helper import (
            get_mixtral_model,
        )
        ...
        self.common_test_model(
            ...
            dynamic=False,
            fullgraph=True,
            onnx_export="test_ort_mixtral_model_backward",
            expected_graph_break=0,
            assert_counting=True,
        )
    ...
    def test_ort_mixtral_model_backward_cuda(self):
        from experimental_experiment.torch_models.mixtral_helper import (
            get_mixtral_model,
        )
    ...
    def test_get_mixtral_model_mask_sdpa(self):
        from experimental_experiment.torch_models.mixtral_helper import (
            get_mixtral_model,
        )

        model, model_inputs = get_mixtral_model(
    ...
    def test_get_mixtral_model_nomask_sdpa(self):
        from experimental_experiment.torch_models.mixtral_helper import (
            get_mixtral_model,
        )
