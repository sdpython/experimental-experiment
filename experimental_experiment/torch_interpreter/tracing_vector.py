...
class TracingShape:
    ...
    def shape_as_tracing_tensor(self):
        ...
        if hasattr(self.tracing_tensor, "__tracing_context__"):
...
class TracingTensor(torch.Tensor):
    ...
    def wraps_with_tracing_tensor(cls, res, context=None):
        ...
        def check_tracing_tensor(t):
            assert hasattr(t, "__INDEX__")
            assert hasattr(t, "__tracing_context__")
            return t
...
class TracingContext:
    ...
    def __init__(self, model: torch.nn.Module, verbose: int = 0):
    ...
    def init_graph(self):
        graph = self.graph = Graph(verbose=self.verbose)
    ...
    def wrap_tensor(self, t):
        ...
        assert self.graph.has_traced_constant(key), (
            f"graph has no key for constant tensor ({type(t)}, {t.dtype}, {t.shape}), "
            f"available={self.graph.traced_constants}"
        )
        return self.graph.get_traced_constant(key)

    def wrap_tracing_tensor(self, t):
        assert isinstance(t, TracingTensor), f"unexpected type {type(t)}"
        if self.graph.has_traced_values(t.__INDEX__):
            return self.graph.get_traced_values(t.__INDEX__)
        if self.graph.has_traced_node(t.__INDEX__):
            return self.graph.has_traced_node(t.__INDEX__)
        # It is a new input.
        self.graph.add_node(
            Node(
                name=self.graph.new_node_name(),
        ...
        assert self.graph.has_traced_values(
            t.__INDEX__
        ), f"graph has no key for traced tensor for index={t.__INDEX__} ({t.dtype}, {t.shape})"
        return self.graph.get_traced_values(t.__INDEX__)

    def wrap_tracing_shape(self, t):
        assert isinstance(t, TracingShape), f"unexpected type {type(t)}"
        source = t.tracing_tensor
        assert self.graph.has_traced_values(source.__INDEX__), (
            f"the shape comes from a for traced tensor missing for the "
            f"graph for index={source.__INDEX__} ({source.dtype}, {source.shape})"
        )

        traced_as = t.shape_traced_as
        if not self.graph.has_traced_node(traced_as.__INDEX__):
        ...
        return self.graph.get_traced_values(traced_as.__INDEX__)
