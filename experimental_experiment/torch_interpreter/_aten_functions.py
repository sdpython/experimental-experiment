...
def _fftn_onnx_normalization(
    ...
    if normalization == 1:
        ...
        result = (
            g.op.Div(transformed, total_sample_count, name=name) if forward else transformed
        )
    else:
        # "backward" - no normalization
        result = (
            transformed if forward else g.op.Mul(transformed, total_sample_count, name=name)
        )
...
def _fftn_onnx(
    ...
    transformed = (
        g.op.DFT(transformed, axis=new_dims[-1], inverse=inverse, onesided=True, name=name)
        if onesided
        else g.op.DFT(
    ...
    transformed = g.op.Squeeze(transformed, axes=np.array([0], dtype=np.int64), name=name)
    g.set_type(transformed, g.get_type(x))
