# modification on the top of https://github.com/microsoft/onnxruntime-genai/tree/main/src/python/py/models
import ast
...
class ModelBuilderBase:
    def __init__(
        ...
        input_shapes: Dict[str, Tuple[Union[str, int], ...]],
        ...
        self.attention_attrs = {
            ...
            "block_sparse": {  # Block-sparse attention-specific variables
                # Sparse block size for SparseAttention op
                "sparse_block_size": sparse_block_size,
                # Kernel block size for sparse attention
                # "kernel_block_size": kernel_block_size,
                # Number of local blocks for sparse attention
                # "local_blocks": local_blocks,
                # Vertical stride to use for sparse attention
                # "vert_stride": vert_block_stride,
                # Use homo head pattern for sparse attention
                # "homo_head": homo_head,
            },
        ...
        self.lm_head_attrs = {
            ...
            # Softcap value to prevent values from exploding in LM head
            # "softcap": lm_head_softcap,
        }
    ...
    def make_node(
        ...
        **kwargs,
        ...
        if name in self.node_names:
            # Note:
            #
            # This approach allows functions that make
            # similar subgraphs with the same naming schema
            # to share existing nodes without needing
            # to know whether the nodes already exist or not
            # (e.g. attention mask subgraphs).
            #
            # This means that the nodes can be created
            # in those functions regardless of their actual
            # status in the graph. This checks can
            # then decide whether the proposed node actually
    ...
    def make_inputs_and_outputs(self):
        ...
        #    key_shape = self.make_key_value_cache_shape(
        #        i, self.input_shapes["past_key_values.key"]
        #    )
        #    inputs.append(
        #        self.make_value(
        #            key_name, dtype=self.input_types["past_key_values.key"], shape=key_shape
        #        )
        #    )
        #    value_name = f"past_key_values.{i}.value"
        ##    value_shape = self.make_key_value_cache_shape(
        #        i, self.input_shapes["past_key_values.value"]
        #    )
        #    inputs.append(
        #        self.make_value(
        #            value_name, dtype=self.input_types["past_key_values.value"],
        #        shape=value_shape
        #        )
        #    )
        ...
        #    outputs.append(
        #        self.make_value(key_name,
        #       dtype=self.output_types["present.key"], shape=key_shape)
        #    )
        ...
        #    outputs.append(
        #        self.make_value(
        #            value_name, dtype=self.output_types["present.value"], shape=value_shape
        #        )
        #    )
    ...
    def make_constant_of_shape(self, name, root_input, value, dtype, shape):
        output = f"{name}/output_0"
        self.make_node(
            "ConstantOfShape", inputs=[root_input], outputs=[output], name=name, value=value
        )
        self.make_value(output, dtype, shape=shape)
    ...
    def make_reduce_mean(self, name, inputs, dtype, shape, keepdims=False):
        output = f"{name}/output_0"
        self.make_node(
            "ReduceMean", inputs=inputs, outputs=[output], name=name, keepdims=keepdims
        )
        self.make_value(output, dtype, shape=shape)
    ...
    def make_matmul_op(self, matmul, basename, root_input, **kwargs):
        if self.onnx_dtype in {ir.DataType.FLOAT16, ir.DataType.BFLOAT16, ir.DataType.FLOAT}:
            ...
            raise NotImplementedError(
                f"The {self.onnx_dtype} precision is not currently supported."
            )
    ...
    def make_matmul_int4(self, matmul, basename, root_input, **kwargs):
        if not hasattr(matmul, "qweight"):
            ...
            # print(f"Saving as {self.io_dtype} on-the-fly
            # and quantizing to {self.onnx_dtype} at the end.")
            return self.make_matmul_float(matmul, basename, root_input, **kwargs)
    ...
    def make_matmul_int4_qdq(self, matmul, matmul_name, root_input, **kwargs):
        ...
        if matmul.bits != 4:
            ...
            raise NotImplementedError(
                f"{matmul.bits} bits precision is not currently supported in QDQ format."
            )

        dequantize_output = self.make_dequantize_linear(f"{matmul_name}/DequantizeLinear", matmul)

        # Add a transpose instead of transposing the weights offline.
        # The reason for this is that it is more natural and usually more performant to
        # compute quantized matmul when the weights are transposed.
        # In most implementations, the transpose should usually be converted to a "transposeB"
        # attribute on the MatMul itself. A more natural way to represent
        # this would have been to use Gemm since it already supports a transB attribute,
        ...
        self.make_transpose(
            transpose_name, dequantize_output, self.io_dtype, transposed_shape, [1, 0]
        )
    ...
    def make_matmul_lora(self, matmul, basename, root_input, **kwargs):
        ...
        matmul_A_name = self.make_matmul_op(
            matmul.lora_A.default, matmul_A_basename, root_input=root_input
        )
        lora_A = f"{matmul_A_name}/output_0"

        matmul.lora_B.default.weight.requires_grad = (
            False  # since a leaf variable is updated in-place
        )
        ...
        matmul_B_name = self.make_matmul_op(
            matmul.lora_B.default, matmul_B_basename, root_input=lora_A
        )
    ...
    def make_packed_matmul(self, q_matmul, k_matmul, v_matmul, basename, root_input, **kwargs):
        if self.onnx_dtype in {ir.DataType.FLOAT, ir.DataType.FLOAT16, ir.DataType.BFLOAT16}:
            return self.make_packed_matmul_float(
                q_matmul, k_matmul, v_matmul, basename, root_input, **kwargs
            )
        elif self.onnx_dtype in {ir.DataType.INT4, ir.DataType.UINT4}:
            return self.make_packed_matmul_int4(
                q_matmul, k_matmul, v_matmul, basename, root_input, **kwargs
            )
        else:
            raise NotImplementedError(
    ...
    def make_packed_matmul_float(
        self, q_matmul, k_matmul, v_matmul, basename, root_input, **kwargs
    ):
        # N_q = num_attention_heads * head_size, N_kv = num_key_value_heads * head_size,
        # H = hidden_size
        # Combine 3 MatMuls of shape N_q x H, N_kv x H, N_kv x H into 1
        # packed MatMul of shape (N_q+N_kv+N_kv)xH
        # Note: Packed MatMul is of shape (N_q+N_kv+N_kv)xH instead
        # of Hx(N_q+N_kv+N_kv) because `make_matmul` will
        ...
        class PackedMatMul:
            def __init__(self):
                self.weight = torch.cat(
                    [q_matmul.weight, k_matmul.weight, v_matmul.weight], dim=0
    ...
    def make_packed_matmul_int4(
        ...
        class PackedMatMul:
            def __init__(self):
                ...
                if (
                    q_matmul.group_size != k_matmul.group_size
                    or q_matmul.group_size != v_matmul.group_size
                ):
                    raise ValueError(
                        "All MatMuls must have the same group size for packed MatMul."
                    )
                self.qweight = torch.cat(
                    [q_matmul.qweight, k_matmul.qweight, v_matmul.qweight], dim=0
                )
                self.scales = torch.cat(
                    [q_matmul.scales, k_matmul.scales, v_matmul.scales], dim=0
                )
                self.qzeros = torch.cat(
                    [q_matmul.qzeros, k_matmul.qzeros, v_matmul.qzeros], dim=0
                )
                self.g_idx = q_matmul.g_idx

                self.in_features = q_matmul.in_features
                self.out_features = (
                    q_matmul.out_features + k_matmul.out_features + v_matmul.out_features
                )
    ...
    def make_embedding(self, embedding):
        ...
        # Use GatherBlockQuantized if and only if tied embeddings are enabled and export model
        ...
        # Use Transpose + Gather for tied embeddings for float embedding layers
            ...
            self.make_node(
                "Gather",
                inputs=[transpose_output, "input_ids"],
                outputs=[gather_output],
                name=gather_name,
            )
        else:
            ...
            gather_output = f"{gather_name}/output_0"
            self.make_node(
                "Gather", inputs=[weight, "input_ids"], outputs=[gather_output], name=gather_name
            )
    ...
    def make_layernorm_op(self, layer_id, layernorm, skip, simple, location):
        ...
        new_io_dtype = (
            ir.DataType.FLOAT if self.layernorm_attrs["cast"]["use_fp32"] else self.io_dtype
        )
        ...
        self.make_initializer(
            layernorm.weight + self.layernorm_attrs["add_offset"], weight, to=new_io_dtype
        )
        ...
        if self.layernorm_attrs["last_layernorm"] and (
        ...
        if cast:
            inputs, outputs = self.make_layernorm_casts(
                name, inputs, outputs, old_io_dtype, new_io_dtype
            )
        ...
        self.make_value(
            outputs[0], new_io_dtype, shape=["batch_size", "sequence_length", self.hidden_size]
        )
        if skip and not self.layernorm_attrs["last_layernorm"]:
            self.make_value(
                outputs[3],
    ...
    def make_layernorm_casts(self, name, inputs, outputs, old_dtype, new_dtype):
        ...
        if (
            self.layernorm_attrs["cast"]["root_input"]
            and self.values[root_input].dtype != new_dtype
        ):
            # Cast root_input
        ...
        if (
            skip
            and self.layernorm_attrs["cast"]["skip_input"]
            and self.values[skip_input].dtype != new_dtype
        ):
            # Cast skip_input
            ...
            self.make_value(
                skip_input_cast_output, new_dtype, shape=self.values[skip_input].shape
            )
        ...
        if (
            skip
            and not self.layernorm_attrs["last_layernorm"]
            and self.layernorm_attrs["cast"]["output_3"]
        ):
            # Cast output_3
    ...
    def make_inv_freq_rescaled_with_ntk(self, inv_freq):
        ...
        low = (
            ...
            * np.log(
                ...
                / (self.rope_attrs["rescale_inv_freq"]["ntk_beta"] * 2 * np.pi)
            )
        ...
        high = (
            ...
            * np.log(
                self.original_context_length
                / (self.rope_attrs["rescale_inv_freq"]["ntk_alpha"] * 2 * np.pi)
            )
            / np.log(self.rope_attrs["theta"])
        )
    ...
    def make_rotary_embedding_caches_from_scratch(self):
        ...
        position_scale = (
            self.rope_attrs["position_scale"]
            if self.context_length == self.original_context_length
            else 1
        )
        t = (
            torch.arange(self.rope_attrs["cache_length"], dtype=self.rope_attrs["t_dtype"])
            * position_scale
        ).type_as(inv_freq)
    ...
    def make_rotary_embedding_caches(self, **kwargs):
        ...
        if self.rope_attrs["create_caches"]:
            ...
            # if hidden dim = head size (i.e. if cos/sin caches haven't been halved yet)
    ...
    def make_padded_cache(self, small_cache, large_cache, pad_value=0.0):
        ...
        This is used for TRT-RTX EP which requires uniform
        dimensions in both branches of If nodes.
    ...
    def _make_split_if_nodes_for_trt_rtx(
        ...
        self.make_value(
            cos_cache_name, self.io_dtype, shape=["max_sequence_length", "head_dim / 2"]
        )
        self.make_value(
            sin_cache_name, self.io_dtype, shape=["max_sequence_length", "head_dim / 2"]
        )
    ...
    def make_rotary_embedding_multi_cache(self, **kwargs):
        ...
        if self.ep == "trt-rtx":
            ...
            cos_cache_small = self.make_padded_cache(
                cos_cache_small, cos_cache_large, pad_value=1.0
            )
            sin_cache_small = self.make_padded_cache(
                sin_cache_small, sin_cache_large, pad_value=0.0
            )
        ...
        # Make the following subgraph to decide
        # which cos/sin caches to use in the rotary embeddings
        #
    ...
    def _make_skip_simplified_layer_norm(
        ...
        self.make_node(
            "Add", inputs=[root_input, skip_input], outputs=[output_3], name=make_add_name
        )
        self.make_value(
            output_3, io_dtype, shape=["batch_size", "sequence_length", self.hidden_size]
        )
    ...
    # This expansion contrib-op can be updated / deprecated in the future.
    def _make_simplified_layer_norm(
        self, basename, root_input, weight_name, output_0, io_dtype, shape
    ):
    ...
    def make_repeat_kv(self, layer_id, root_input, past_kv, present_kv, **kwargs):
        # Make subgraph that repeats tensor of shape
        # (batch_size, sequence_length, num_kv_heads, head_size)
        # to shape (batch_size, sequence_length, num_attn_heads, head_size)
        ...
        basename = (
            f"/model/layers."
            f"{layer_id}/attn/{'k_proj' if past_kv.endswith('key') else 'v_proj'}/repeat_kv"
        )
        ...
        self.make_node(
            "Concat", inputs=concat_1_inputs, outputs=[present_kv], name=concat_1_name, axis=2
        )
        ...
        self.make_concat(
            concat_2_name, concat_2_inputs, dtype=ir.DataType.INT64, shape=[5], axis=0
        )
        ...
        self.make_concat(
            concat_3_name, concat_3_inputs, dtype=ir.DataType.INT64, shape=[4], axis=0
        )
    ...
    def make_attention_op(self, name, **kwargs):
        ...
        if op_type == "MultiHeadAttention":
            self.make_multi_head_attention(
                name, add_qk=f"{self.mask_attrs['mask_name']}/output_0", **kwargs
            )
    ...
    def make_group_query_attention(self, name, **kwargs):
        ...
        sinks = kwargs.get(
            "sinks", ""
        )  # TODO: add to inputs list directly once ORT 1.23 is out (one-time exception)
        if sinks:
            inputs += [sinks]
    ...
    def make_attention_input_proj(self, layer_id, attention, root_input, **kwargs):
        ...
        q_bias_exists = (
            attention.q_proj.bias is not None and torch.count_nonzero(attention.q_proj.bias) > 0
        )
        k_bias_exists = (
            attention.k_proj.bias is not None and torch.count_nonzero(attention.k_proj.bias) > 0
        )
        v_bias_exists = (
            attention.v_proj.bias is not None and torch.count_nonzero(attention.v_proj.bias) > 0
        )
        ...
        if self.attention_attrs["use_packed_matmul"] and qkv_dtype_equal and any_bias_exists:
            ...
            if k_bias_exists:
                ...
                self.make_add_bias(
                    attention.k_proj.bias, k_add_name, root_input=self.attention_attrs["k_path"]
                )
                self.attention_attrs["k_path"] = f"{k_add_name}/output_0"
            if v_bias_exists:
                v_add_name = f"/model/layers.{layer_id}/attn/v_proj/Add"
                self.make_add_bias(
                    attention.v_proj.bias, v_add_name, root_input=self.attention_attrs["v_path"]
                )
    ...
    def make_attention_qk_subgraph(self, layer_id, attention, root_input, **kwargs):
        ...
        # Make repeat KV nodes (Note: `repeat_kv` needs to be kept
        # since GroupQueryAttention isn't supported for FP32 CUDA)
    ...
    def make_attention_output_proj(self, layer_id, attention, root_input, **kwargs):
        ...
        self.layernorm_attrs["skip_input"] = (
            f"{o_matmul_name if not o_bias_exists else o_add_name}/output_0"
        )

    def make_attention_unpacked(self, layer_id, attention, root_input, **kwargs):
        qkv_linear = getattr(attention, "qkv_proj", None) or getattr(
            attention, "query_key_value", None
        )
        ...
        if hasattr(qkv_linear, "base_layer"):
            ...
            self.make_attention_unpacked_lora(
                layer_id, attention, qkv_linear, root_input, **kwargs
            )
        else:
            # For regular packed `MatMul`
            self.make_attention_unpacked_regular(
                layer_id, attention, qkv_linear, root_input, **kwargs
            )
    ...
    def make_attention_unpacked_lora(self, layer_id, attention, qkv_linear, root_input, **kwargs):
        ...
        k_proj.bias = (
            ...
            if qkv_linear.bias is None
                ...
                qkv_linear.bias[q_size : q_size + kv_size], requires_grad=False
            )
        )
        ...
        q_lora_B.bias = (
            ...
            else torch.nn.Parameter(lora_B.bias[:q_size], requires_grad=False)
        )

        k_lora_B = torch.nn.Linear(in_features=q_size, out_features=kv_size)
        k_lora_B.weight = torch.nn.Parameter(
            lora_B.weight[q_size : q_size + kv_size, :], requires_grad=False
        )
        k_lora_B.bias = (
            None
            if lora_B.bias is None
        ...
        v_lora_B.weight = torch.nn.Parameter(
            lora_B.weight[q_size + kv_size :, :], requires_grad=False
        )
        v_lora_B.bias = (
            None
            if lora_B.bias is None
    ...
    def make_attention_unpacked_regular(
        self, layer_id, attention, qkv_linear, root_input, **kwargs
    ):
        q_size = self.num_attn_heads * self.head_size
        kv_size = self.num_kv_heads * self.head_size
        ...
        attention.q_proj.weight = torch.nn.Parameter(
            qkv_linear.weight[:q_size, :], requires_grad=False
        )
        attention.q_proj.bias = (
            None
            if qkv_linear.bias is None
    ...
    def make_mlp_unpacked_lora(self, layer_id, mlp, gate_up_linear, root_input):
        ...
        gate_proj.bias = (
            ...
            if gate_up_linear.bias is None
                ...
                gate_up_linear.bias[: self.intermediate_size], requires_grad=False
            )
        )
    ...
    def make_mlp_unpacked_regular(self, layer_id, mlp, gate_up_linear, root_input):
        ...
        mlp.up_proj = torch.nn.Linear(
            in_features=self.hidden_size, out_features=self.intermediate_size
        )
        mlp.up_proj.weight = torch.nn.Parameter(
            gate_up_linear.weight[self.intermediate_size :, :]
        )
        mlp.up_proj.bias = (
            None
            if gate_up_linear.bias is None
    ...
    def make_mlp_proj(self, layer_id, mlp, root_input):
        ...
        gate_bias_exists = (
            mlp.gate_proj.bias is not None and torch.count_nonzero(mlp.gate_proj.bias) > 0
        )
        up_bias_exists = (
            mlp.up_proj.bias is not None and torch.count_nonzero(mlp.up_proj.bias) > 0
        )
        down_bias_exists = (
            mlp.down_proj.bias is not None and torch.count_nonzero(mlp.down_proj.bias) > 0
        )
        ...
        if gate_bias_exists:
            ...
            self.make_add_bias(
                mlp.gate_proj.bias, gate_add_name, root_input=f"{gate_name}/output_0"
            )
        ...
        down_matmul_name = self.make_matmul(
            mlp.down_proj, down_matmul_basename, f"{mul_name}/output_0"
        )
        down_name = down_matmul_name
        if down_bias_exists:
            down_add_name = f"/model/layers.{layer_id}/mlp/down_proj/Add"
            self.make_add_bias(
                mlp.down_proj.bias, down_add_name, root_input=f"{down_name}/output_0"
            )
    ...
    def make_mlp_fc(self, layer_id, mlp, root_input):
        ...
        fc2_matmul_name = self.make_matmul(
            mlp.fc2, fc2_matmul_basename, root_input=f"{act_fn_name}/output_0"
        )
    ...
    def make_base_moe_op(self, name, **kwargs):
        ...
        extra_kwargs = (
            ...
            if self.moe_attrs["swiglu_limit"] is not None
            else {}
        )
        self.make_node(
            "MoE",
        ...
        self.make_value(
            output, self.io_dtype, shape=["batch_size", "sequence_length", self.hidden_size]
        )
    ...
    def make_qmoe_weights(self, weights):
        ...
        use_blockwise_quant = "int4_block_size" in self.extra_options and self.ep in [
            "cpu",
            "webgpu",
        ]
        ...
        if use_blockwise_quant:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Block-wise quantization failed with block_size={block_size}: {e}"
                ) from e
        ...
        try:
            ...
            print(
                "WARNING: TensorRT-LLM is needed to use torch.ops."
                "trtllm._symmetric_quantize_last_axis_of_batched_matrix()."
            )
        except RuntimeError as r:
            print(
                "WARNING: TensorRT-LLM failed to run torch.ops.trtllm."
                "_symmetric_quantize_last_axis_of_batched_matrix() successfully."
            )
            err = str(r)
            ...
            if unsuccessful:
                raise RuntimeError(
                    "Failed to quantize MoE weights with TensorRT-LLM. Please ensure "
                    "TensorRT-LLM installs and runs successfully in your environment."
                )
    ...
    def make_block_sparse_moe(self, layer_id, bsm, root_input, output):
    ...
    def make_activation_with_mul(self, layer_id, root_input, activation, domain):
        ...
        self.make_node(
            activation, inputs=[root_input], outputs=[act_output], name=act_name, domain=domain
        )
        self.make_value(
            act_output,
            dtype=self.io_dtype,
    ...
    def make_gelu(self, layer_id, root_input, activation):
        ...
        if activation == "Gelu":
            self.make_node(
                "Gelu", inputs=[root_input], outputs=[output], name=gelu_name, approximate="none"
            )
        elif activation == "FastGelu":
            self.make_node(
                "Gelu", inputs=[root_input], outputs=[output], name=gelu_name, approximate="tanh"
            )
        else:
            self.make_node(
                activation,
    ...
    def make_relu(self, layer_id, root_input, activation):
        ...
        self.make_node(
            activation, inputs=[root_input], outputs=[output], name=relu_name, domain=""
        )
        self.make_value(
            output, self.io_dtype, shape=["batch_size", "sequence_length", self.intermediate_size]
        )
    ...
    def make_relu_squared(self, layer_id, root_input, activation):
        ...
        self.make_node(
            "Pow", inputs=pow_inputs, outputs=[f"{pow_name}/output_0"], name=pow_name, domain=""
        )
        self.make_value(
            f"{pow_name}/output_0",
            self.io_dtype,
    ...
    def make_lm_head(self, lm_head):
        ...
        matmul_name = self.make_matmul(
            lm_head, matmul_basename, root_input, logits=not any(exists_checks)
        )
        lm_name = matmul_name
    ...
    def make_attention_mask_reformatting_for_mha(self):
        ...
        end_where_name = self.make_attention_mask_subgraph(
            attn_mask_basename, shared_unsqueeze_name
        )
    ...
    def make_input_ids_subgraph(self, basename, past_key_gather_name):
        # Make shared nodes between past_key_values.0.key (Gather with idx=2)
        # and input_ids (Gather with idx=1) subgraphs
        #
        #       Gather          Gather
        #       (idx=1)         (idx=2)
        #              \       /
        ...
        unsqueeze_4_name = f"{basename}/Unsqueeze_4"
        self.make_unsqueeze(
            unsqueeze_4_name, unsqueeze_inputs, dtype=ir.DataType.INT64, shape=[1]
        )
        unsqueeze_5_name = f"{basename}/Unsqueeze_5"
        self.make_unsqueeze(
            unsqueeze_5_name, unsqueeze_inputs, dtype=ir.DataType.INT64, shape=[1]
        )
        unsqueeze_6_name = (
            f"{basename}/Unsqueeze_6"  # shared unsqueeze for input_ids and attention_mask
        )
        self.make_unsqueeze(
            unsqueeze_6_name, unsqueeze_inputs, dtype=ir.DataType.INT64, shape=[1]
        )
        concat_2_name = f"{basename}/Concat_2"
        ...
        constant_shape_value = ir.tensor(
            torch.tensor(
                [torch.finfo(constant_shape_torch_dtype).min], dtype=constant_shape_torch_dtype
            ),
        ...
        self.make_unsqueeze(
            unsqueeze_7_name, unsqueeze_7_inputs, dtype=ir.DataType.INT64, shape=[1]
        )
        concat_3_name = f"{basename}/Concat_3"
        concat_3_inputs = [f"{unsqueeze_7_name}/output_0", "/model/constants/INT64/[1]"]
        self.make_concat(
            concat_3_name, concat_3_inputs, dtype=ir.DataType.INT64, shape=[2], axis=0
        )
    ...
    def make_attention_mask_subgraph(self, basename, unsqueeze_for_concat):
        ...
        # ['batch_size', 'total_sequence_length'] --> ['batch_size', 1, 'total_seq_length']
        ...
        # ['batch_size', 1, 'total_sequence_length'] --> ['batch_size', 1, 1, 'total_seq_length']
        attention_mask_shape.insert(1, 1)
        self.make_unsqueeze(
            unsqueeze_4_name,
            unsqueeze_4_inputs,
        ...
        self.make_sub(
            sub_name, sub_inputs, dtype=self.io_dtype, shape=["unk", "unk", "unk", "unk"]
        )
        ...
        self.make_where(
            where_2_name, where_2_inputs, dtype=self.io_dtype, shape=["unk", "unk", "unk", "unk"]
        )
    ...
    def make_common_mask_reformat_subgraph(
        ...
        constant_shape_name = (
            f"{basename}/ConstantOfShape"
            if not input_ids_subgraph
            else f"{basename}/ConstantOfShape_1"
        )
        constant_shape_value = ir.tensor([1], dtype=ir.DataType.INT64)
        self.make_constant_of_shape(
            constant_shape_name,
    ...
    def make_attention_mask_graph_capture_reformatting_for_gqa(self, attn_mask_basename):
        ...
        self.make_reduce_sum(
            reduce_sum_name, reduce_sum_inputs, dtype=ir.DataType.INT32, shape=["batch_size"]
        )
        ...
        self.make_reduce_max(
            reduce_max_name, reduce_max_inputs, dtype=ir.DataType.INT32, shape=[]
        )
    ...
    def make_attention_mask_standard_reformatting_for_gqa(self, attn_mask_basename):
        ...
        self.make_reduce_sum(
            reduce_sum_name, reduce_sum_inputs, dtype=ir.DataType.INT64, shape=["batch_size"]
        )
        sub_name = f"{attn_mask_basename}/Sub"
        ...
        self.make_cast(
            cast_1_name, f"{sub_name}/output_0", dtype=ir.DataType.INT32, shape=["batch_size"]
        )
        ...
        self.make_cast(
            cast_2_name, f"{gather_name}/output_0", dtype=ir.DataType.INT32, shape=None
        )
