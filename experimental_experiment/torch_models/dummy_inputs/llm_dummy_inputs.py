...
def restore_dummy_inputs_for_phi35_vision_instruct(
    ...
    func:`create_dummy_inputs_for_phi35_vision_instruct`.
    """
    ...
    assert os.path.exists(filename), (
        ...
        f"create_dummy_inputs_for_phi35_vision_instruct to create them"
    )
    return create_input_tensors_from_onnx_model(filename, device=device, engine="onnxruntime")


def create_dummy_inputs_for_phi35_vision_instruct(
    num_hidden_layers: int = 2,
    with_images: bool = False,
    device: str = "cpu",
    verbose: int = 0,
    ...
    It can be restored by :func:`restore_dummy_inputs_for_phi35_vision_instruct`.
    """
    ...
    if verbose:
        print(
            f"[create_dummy_inputs_for_phi35_vision_instruct] "
    ...
    if with_images:
        for i in range(1, 3):
            ...
            print(f"[create_dummy_inputs_for_phi35_vision_instruct] image size {img.size}")
            images.append(img)
    ...
    if verbose:
        print("[create_dummy_inputs_for_phi35_vision_instruct] create inputs")
    inputs = processor(prompt, images if with_images else None, return_tensors="pt").to(device)
    if verbose:
        print(
            f"[create_dummy_inputs_for_phi35_vision_instruct] types: "
