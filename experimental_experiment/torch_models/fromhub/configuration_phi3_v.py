...
class Phi3VConfig(PretrainedConfig):
    ...
    [`Phi3VModel`]. It is used to instantiate a Phi-3
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 32064):
            Vocabulary size of the Phi-3-V model.
        ...
        rope_scaling (`dict`, *optional*):
            ...
            contain the following keys: `type`, `short_factor` and `long_factor`.
            The `type` must be either `su` or `yarn` and
            the `short_factor` and `long_factor` must be lists of numbers
            with the same length as the hidden size
        ...
        embd_layer (`str`, *optional*, defaults to `"default"`):
            The embedding layer to use. Can be either `"default"` or `"image"`.
            "default" uses the standard embedding for text.
    ...
    def _rope_scaling_validation(self):
        ...
        if not isinstance(self.rope_scaling, dict) or len(self.rope_scaling) != 3:
            raise ValueError(
                "`rope_scaling` must be a dictionary with three fields, "
                f"`type`, `short_factor` and `long_factor`, "
        ...
        if rope_scaling_type is None or rope_scaling_type not in ["su", "yarn"]:
            raise ValueError(
                f"`rope_scaling`'s type field must be one of ['su', 'yarn'], "
        ...
        if not (
            ...
            raise ValueError(
                f"`rope_scaling`'s short_factor field must be a list "
                f"of numbers, got {rope_scaling_short_factor}"
            )
        if (
        ...
        if not (
            ...
            raise ValueError(
                f"`rope_scaling`'s long_factor field must be a list of numbers, "
                f"got {rope_scaling_long_factor}"
            )
        if (
