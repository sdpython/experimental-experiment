...
class Phi3ImageEmbedding(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, wte=None, **kwargs) -> None:
        ...
        if hasattr(config, "embd_pdrop") or hasattr(config, "embed_pdrop"):
            embd_drop = (
                config.embd_pdrop if hasattr(config, "embd_pdrop") else config.embed_pdrop
            )
        ...
        if self.with_learnable_separator:
            ...
            logger.info(
                "learnable separator enabled for hd transform, hd_transform_order = %s",
                self.hd_transform_order,
            )
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    Args:
        ...
        unsqueeze_dim (`int`, *optional*, defaults to 1):
            ...
            cos[position_ids] and sin[position_ids] broadcastable
            to the shapes of q and k. Similarly, if q and k have
            the shape [batch_size, seq_len, heads, head_dim],
    ...
    def __init__(self, config: Phi3VConfig, layer_idx: Optional[int] = None):
        ...
        if layer_idx is None:
            logger.warning_once(
                ...
                "lead to errors during the forward call if caching is used. "
...
class Phi3FlashAttention2(Phi3Attention):
    ...
    `Phi3Attention` as the weights of the module stays
    untouched. The only required change would be on the
    forward pass where it needs to correctly call the public API of
    ...
    def __init__(self, *args, **kwargs):
        ...
        # that was made default for flash_attn>=2.1. This attribute is used to handle
        # this difference. Reference: https://github.com/Dao-AILab/flash-attention/releases/tag/v2.1.0.
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not _flash_supports_window_size:
            logger.warning_once(
                "The current flash attention version does not "
                "support sliding window attention. "
                "Please use `attn_implementation='eager'` or "
                "supgrade flash-attn library."
            )
            raise ValueError(
                "The current flash attention version does not "
                "support sliding window attention."
        ...
        # Because the input can be padded, the absolute sequence
        # length depends on the max position id.
            ...
            if (
                ...
                if past_key.shape[-2] != self.config.sliding_window - 1:
                    raise ValueError(
                        ...
                        f"self.config.sliding_window-1, head_dim`), got"
                        f" {past_key.shape}"
                    )
        ...
        # This might slowdown training & inference so it is
        # recommended to not cast the LayerNorms
        # in fp32.
    ...
    # MistralFlashAttention2._flash_attention_forward
    def _flash_attention_forward(
        ...
        first unpad the input, then computes the attention scores
        and pad the final attention scores.
    ...
    # Copied from transformers.models.mistral.modeling_mistral.
    # MistralFlashAttention2._upad_input
...
class Phi3SdpaAttention(Phi3Attention):
    """
    Phi3 attention module using torch.nn.functional.scaled_dot_product_attention.
    This module inherits from
    `Phi3Attention` as the weights of the module stays untouched.
    ...
    def forward(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "Phi3Model is using Phi3SdpaAttention, but "
                ...
                "Transformers version v5.0.0 onwards. This warning can be removed "
                "using the argument `attn_implementation='eager'` when loading the model."
            )
...
class Phi3DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if "padding_mask" in kwargs:
            warnings.warn(
                "Passing `padding_mask` is deprecated and will be "
                "removed in v4.37. Please make sure use `attention_mask` instead.`",
                stacklevel=0,
            )
        """
        Args:
            ...
            position_ids (`torch.LongTensor` of shape `({0})`, *optional*):
                ...
                in the position embeddings. Selected in the range
            ...
            use_cache (`bool`, *optional*):
                If set to `True`, `past_key_values`
...
PHI3V_INPUTS_DOCSTRING = r"""
    Args:
        ...
        attention_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            [the paper](https://arxiv.org/abs/1910.13461) for more
            information on the default strategy.

            - 1 indicates the head is **not masked**,
            - 0 indicates the head is **masked**.
        position_ids (`torch.LongTensor` of shape
            `(batch_size, sequence_length)`, *optional*):
            Indices of positions of each input sequence tokens
            in the position embeddings. Selected in the range `[0,
            config.n_positions - 1]`.
            [What are position IDs?](../glossary#position-ids)
        past_key_values (`Cache` or `tuple(tuple(torch.FloatTensor))`, *optional*):
            Pre-computed hidden-states (key and values in the self-attention
            blocks and in the cross-attention
            ...
            If no `past_key_values` are passed, the
            ...
            `(batch_size, 1)` instead of all `input_ids`
        ...
        pixel_values (`torch.FloatTensor` of shape
            ...
            Pixel values can be obtained using [`AutoImageProcessor`].
...
class Phi3VModel(Phi3VPreTrainedModel):
    ...
    layers. Each layer is a [`Phi3DecoderLayer`]
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if pixel_values is not None and image_sizes is not None:
                assert (
                    self.vision_embed_tokens is not None
                ), "Vision embedding layer is not defined"
        ...
        if (
            ...
            if is_padding_right:
                raise ValueError(
                    ...
                    "Flash Attention version of Phi3. Make sure to "
                    " call `tokenizer.padding_side  = 'left'` "
...
class Phi3VForCausalLM(Phi3VPreTrainedModel):
    ...
    # LlamaForCausalLM.__init__ with Llama->Phi3
    def __init__(self, config):
        super().__init__(config)
    ...
    # Copied from transformers.models.llama.modeling_llama.
    # LlamaForCausalLM.set_input_embeddings
    def set_input_embeddings(self, value):
        self.model.embed_tokens = value

    # Copied from transformers.models.llama.modeling_llama.
    # LlamaForCausalLM.get_output_embeddings
    def get_output_embeddings(self):
        return self.lm_head

    # Copied from transformers.models.llama.modeling_llama.
    # LlamaForCausalLM.set_output_embeddings
    ...
    def forward(
        ...
        'This is an example script .\n Certainly! Below is a sample
        script that demonstrates a simple task, such as calculating the sum'
        ```"""
    ...
    # Copied from transformers.models.persimmon.modeling_persimmon.
    # PersimmonForCausalLM.prepare_inputs_for_generation
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # When the first time input length reached long and short factor
        # switching point, enforce re-compute cache
        # It will cause downside of slower at this single token position,
        # however, better than current failure.
        if (
            past_key_values
        ...
        if past_key_values is not None:
            ...
            # 1 - If the length of the attention_mask exceeds the length
            # of input_ids, then we are in a setting where
            # some of the inputs are exclusively passed as part of
            # the cache (e.g. when passing input_embeds as
            # input)
...
@add_start_docstrings(
    ...
    [`Phi3VForSequenceClassification`] uses the last token in order
    to do the classification, as other causal models (e.g. GPT-2) do.

    Since it does classification on the last token,
    it requires to know the position of the last token. If a
    `pad_token_id` is defined in the configuration,
    it finds the last token that is not a padding token in each row. If
    no `pad_token_id` is defined, it simply takes the last value in each row
    of the batch. Since it cannot guess the
    padding tokens when `inputs_embeds` are passed instead of `input_ids`,
...
# with Llama->Phi3, LLAMA->PHI3, self.transformer->self.model,
# transformer_outputs->model_outputs
class Phi3VForSequenceClassification(Phi3VPreTrainedModel):
    ...
    def forward(
        ...
        if self.config.pad_token_id is None:
            ...
            if input_ids is not None:
                # if no pad token found, use modulo instead
                # of reverse indexing for ONNX compatibility
                sequence_lengths = (
...
@add_start_docstrings(
    """
    [`Phi3VModel`] with a token classification head on top
